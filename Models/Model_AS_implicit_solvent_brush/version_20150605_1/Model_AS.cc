...
void Model_AS::update(){
    ...
    arr = 1.0/sqrt(2.0*kPI*alpha) * exp(-x*x/2/alpha);  // x0=0:grafted at left surface,x0=_cfg.a():grafted at right surface
    qc = 0.0;
    for(int i=0; i<_cfg.Lx(); ++i) {
        delta(i, all, all) = arr(i);
        qc(0, i, all, all) = 2 * graft_density * delta(i, all, all) / q(sA-1, 0, all, all);
    }
...
double Model_AS::Hw() const{
    double ret;
    Grid g = 0.5 * v * C * C * (*phiA) * (*phiA) - (*wA) * (*phiA);
         ret = g.quadrature();
    return ret;
}
