...
void Model_A_B::init(){
    ...
    sC = vMs(1);
    dsA = 1.0/(sA-1); 
    dsC = _cfg.length_ratio()/(sC-1); //length_ratio is length ration of grafted homopymer C over confined homopolymer A
...
void Model_A_B::update(){
    ...
    phiA->set_cc(fA/QA);   
    phiA->update(*qA, *qAc);
    phiC->set_cc(1.0/_cfg.length_ratio());
    phiC->update(*qC, *qCc);
...
/***************************calculate specific segment**********************************/
    ...
    phia_end(all, all, all) = fA/QA*qsA(0, all, all, all)*qsAc(sA-1, all, all, all);
    phia_middle(all, all, all) = fA/QA*qsA((sA-1)/2, all, all, all)*qsAc((sA-1)/2, all, all, all); 
    ...
    phic_begin(all, all, all) = qsC(0, all, all, all)*qsCc(sC-1, all, all, all)/_cfg.length_ratio();
    phic_end(all, all, all) = qsC(sC-1, all, all, all)*qsCc(0, all, all, all)/_cfg.length_ratio();
    phic_middle(all, all, all) = qsC((sC-1)/2, all, all, all)*qsCc((sC-1)/2, all, all, all)/_cfg.length_ratio();
}
...
double Model_A_B::Hs() const{
	double Hs1 = -fA*log(qA->Qt())-fC/_cfg.length_ratio()*log(QC);
    double Hs2 = (phiC->quadrature())/_cfg.length_ratio()*(log((phiC->quadrature())/_cfg.length_ratio())-1.0);
    double Hs3 = (phiA->quadrature())*log((phiA->quadrature())/_cfg.length_ratio());
    return Hs1 + Hs2 + Hs3;
}
