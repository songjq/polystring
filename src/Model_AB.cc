...
blitz::Array<double, 4> Model_AB::output_data() {
    blitz::Range all = blitz::Range::all();
    blitz::Array<double, 4> dataSave(4, _cfg.Lx(), _cfg.Ly(), _cfg.Lz());
    dataSave(0, all, all, all) = wA->data();
    dataSave(1, all, all, all) = wB->data();
    dataSave(2, all, all, all) = phiA->data();
    dataSave(3, all, all, all) = phiB->data();
...
void Model_AB::release_memory_string(){
    if(_cfg.algo_scft_type() == AlgorithmSCFTType::ANDERSON){
        delete wAx;
        delete wBx;
    }
