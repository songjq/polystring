...
/*define wall density phiW, wall potential H and polymer volume fraction phi*/
    ...
    for(int i=0; i<Lx; ++i) {
        /*if(i<L_tau) {
            EXP_array(i) = exp(4*tau*x(i)/(tau*tau-x(i)*x(i)));
            array1(i) = 1.0-pow2(EXP_array(i)-1)/pow2(EXP_array(i)+1);
            array2(i) = 0.2*(1.0-pow2(EXP_array(i)-1)/pow2(EXP_array(i)+1));   //0.2 should be variable
        }
        else if(i<Lx-L_tau) {
            array1(i) = 0.0;
            array2(i) = 0.0;
            ...
            EXP_array(i) = exp(4*tau*(La*(1.0-1.0/Lx)-x(i))/(tau*tau-pow2(La*(1.0-1.0/Lx)-x(i))));
            array1(i) = 1.0-pow2(EXP_array(i)-1)/pow2(EXP_array(i)+1);
            array2(i) = 0.2*(1.0-pow2(EXP_array(i)-1)/pow2(EXP_array(i)+1));  //0.2 should be variable
        }*/
        /*if(i<L_depletion) {
