...
void Model_AB_C::init(){
    ...
    switch (_cfg.dim()) {
        case 1 :
            sigma = fC * _cfg.a() / eps;  // grafted density: sigma is \sigma*N/\rho_0 = fC * La / eps actually
            break;
        case 2 :
            sigma = fC * _cfg.b() / eps;
            break;
...
void Model_AB_C::update(){
    ...
    switch(_cfg.dim()) {
        case 1:
        {
            ...
            Cheb cheb(_cfg.Lx());
            QC = 0.5 * cheb.quadrature_clencurt(v); //Notice:QC here is already normalized by _cfg.a().
    ...
    qC->update(*wC);
    QAB = qB->Qt();
    phiA->set_cc((1.0-fC)/QAB); 
    //phiA->set_cc((1.0-fC));   
    phiA->update(*qA, *qAc);
    phiB->set_cc((1.0-fC)/QAB);
    //phiB->set_cc((1.0-fC));   
    phiB->update(*qB, *qBc);
    phiC->set_cc(1.0); // do not forget to divide eps
    phiC->update(*qC, *qCc);
    ...
    if(confine_type == ConfineType::NONE){
        if(_cfg.algo_mde_type() == AlgorithmMDEType::ETDRK4
           && _cfg.etdrk4_M() <= 0){
            ...
            qCc = new Propagator("qCc", _cfg, sC, dsC, one);  //qC is complementary propagator is not initialized here
        }
