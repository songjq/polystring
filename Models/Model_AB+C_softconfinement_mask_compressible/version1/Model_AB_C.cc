...
void Model_AB_C::init(){
    ...
    XiN = _cfg.get_double("Model","XiN");
    chiNaw = _cfg.get_double("Model","chiNaw");
    chiNbw = _cfg.get_double("Model","chiNbw");
    chiNcw = _cfg.get_double("Model","chiNcw");
    vec vf = _cfg.f();
    fA = vf(0);
    fB = vf(1);
    fC = vf(2);
...
void Model_AB_C::update(){
    ...
    phiA->set_cc(phi*(1.0-fC));   
    phiA->update(*qA, *qAc);
    //phiB->set_cc((1.0-fC/QAB));
    phiB->set_cc(phi*(1.0-fC));   
    phiB->update(*qB, *qBc);
    phiC->set_cc(phi*1.0/eps);
    phiC->update(*qC, *qCc);

    //yita->update( (*phiA) + (*phiB) + (*phiC) - 1.0 );
    wA->update( chiNac * (*phiC) + chiNab * (*phiB) + chiNaw * (*phiW) + XiN*((*phiA)+(*phiB)+(*phiC)+(*phiW)-1.0));
    wB->update( chiNab * (*phiA) + chiNbc * (*phiC) + chiNbw * (*phiW) + XiN*((*phiA)+(*phiB)+(*phiC)+(*phiW)-1.0));
    wC->update( chiNac * (*phiA) + chiNbc * (*phiB) + chiNcw * (*phiW) + XiN*((*phiA)+(*phiB)+(*phiC)+(*phiW)-1.0)); 
}

double Model_AB_C::Hw() const{
    double ret;
    Grid g = chiNac * (*phiA) * (*phiC) + chiNab * (*phiA) * (*phiB) + chiNbc * (*phiB) * (*phiC) +
             chiNaw * (*phiA) * (*phiW) + chiNbw * (*phiB) * (*phiW) + chiNcw * (*phiC) * (*phiW) + 
             XiN/2.0*((*phiA)+(*phiB)+(*phiC)+(*phiW)-1.0)*((*phiA)+(*phiB)+(*phiC)+(*phiW)-1.0)  -
...
double Model_AB_C::Hs() const{
    ...
    return -phi*(1.0-fC)*log(QAB) - 1.0*phi*fC/eps*log(QC);
}
...
double Model_AB_C::residual_error() const{
    ...
    Grid g1, g2;
    // wA
    g1 = chiNac * (*phiC) + chiNab * (*phiB) + chiNaw * (*phiW) + XiN*((*phiA)+(*phiB)+(*phiC)+(*phiW)-1.0);
    g2 = g1 - (*wA);
    r = g2.abs_mean();
    x = wA->abs_mean();
    b = g1.abs_mean();
    res += r / (x+b);
    // wB
    g1 = chiNab * (*phiA) + chiNbc * (*phiC) + chiNbw * (*phiW) + XiN*((*phiA)+(*phiB)+(*phiC)+(*phiW)-1.0);
    g2 = g1 - (*wB);
    r = g2.abs_mean();
    x = wB->abs_mean();
    b = g1.abs_mean();
    res += r / (x+b);
    // wC
    g1 = chiNac * (*phiA) + chiNbc * (*phiB) + chiNcw * (*phiW) + XiN*((*phiA)+(*phiB)+(*phiC)+(*phiW)-1.0);
    g2 = g1 - (*wC);
    r = g2.abs_mean();
    x = wC->abs_mean();
    b = g1.abs_mean();
...
void Model_AB_C::display() const{
    ...
    cout<<"\tphiW = "<<phiW->quadrature();
    cout<<"\t["<<phiW->min()<<", "<<phiW->max()<<"]"<<endl;
...
void Model_AB_C::save_model(const string file){
    ...
    if(!mat.queryStatus()){
        ...
        mat.matPutScalar("chiNaw", chiNbc);
        mat.matPutScalar("chiNbw", chiNbc);
        mat.matPutScalar("chiNcw", chiNbc);
        mat.matPutScalar("XiN", XiN);
        mat.matPutScalar("d", d);
...
/*****************calculate delta function**********************/
    ...
    switch(_cfg.dim()) {
        ...
        {                              // brackets is necessary for initialization in switch case
            ...
            vec du = -D * u;
            vec du2 = du % du;
            double du2_sum = 0.5 * (_cfg.a()) * cheb.quadrature_clencurt(du2);
            du2 = du2 / du2_sum / 2.0;  //half delta function is need thus du2 must be divided by 2
            for(int i=0; i<Lx; ++i)   
                delta(i, all, all) = du2(i);
            
            break;
        }
        case 2:
        {                              
            vec u = linspace(0, Ly-1, Ly);
            for (int j=0; j<Ly; j++)
                u(j) = phiw(0, j, 0);
            Cheb cheb(Ly);
            mat D = cheb.D();
            vec du = -D * u;
            vec du2 = du % du;
            double du2_sum = 0.5 * (_cfg.b()) * cheb.quadrature_clencurt(du2);
            du2 = du2 / du2_sum / 2.0;
            for(int j=0; j<Ly; j++)   
                delta(all, j, all) = du2(j);
            break;
        }
        case 3:
        {                              
            vec u = linspace(0, Lz-1, Lz);
            for (int k=0; k<Lz; k++)
                u(k) = phiw(0, 0, k);
            Cheb cheb(Lz);
            mat D = cheb.D();
            vec du = -D * u;
            vec du2 = du % du;
            double du2_sum = 0.5 * (_cfg.c()) * cheb.quadrature_clencurt(du2);
            du2 = du2 / du2_sum / 2.0;
            for(int k=0; k<Lz; k++)   
                delta(all, all, k) = du2(k);
            break;
        }
        default:
            break;
...
/*define wall density phiW, wall potential H and polymer volume fraction phi*/
    ...
    d = _cfg.depletion_length();
    int Ld; //grids for mask
    ...
    switch (_cfg.dim()) {
        case 1:
        {
            ...
            blitz::Array<double, 1> array(Lx);
            blitz::firstIndex j;
            array = 0.0;
            x = cos(kPI*j/(Lx-1));
            x = (1.0-x)/2.0*La;
            Ld = (int)((Lx-1)/kPI*acos(1-2.0*d/La));
            //d = (1-cos(kPI*Ld/(Lx-1)))/2.0*La;
            for(int i=0; i<Ld; ++i) 
                array(i) = (1.0+cos(kPI*x(i)/d))/2.0;
            for(int i=0; i<Lx; ++i)
                for(int j=0; j<Ly; ++j)
                    for(int k=0; k<Lz; ++k) {        
                        phiw(i, j, k) = array(i);
        ...
        case 2:
        {
            ...
            blitz::Array<double, 1> array(Ly);
            blitz::firstIndex j;
            array = 0.0;
            y = cos(kPI*j/(Ly-1));
            y = (1-y)/2.0*Lb;
            Ld = (int)((Ly-1)/kPI*acos(1-2.0*d/Lb));
            //d = (1-cos(kPI*Ld/(Ly-1)))/2.0*Lb;
            for(int i=0; i<Ld; ++i) 
                array(i) = (1.0+cos(kPI*y(i)/d))/2.0;
            for(int i=0; i<Lx; ++i)
                for(int j=0; j<Ly; ++j)
                    for(int k=0; k<Lz; ++k) {        
                        phiw(i, j, k) = array(j);
        ...
        case 3:
        {
            ...
            blitz::Array<double, 1> array(Lz);
            blitz::firstIndex j;
            array = 0.0;
            z = cos(kPI*j/(Lz-1));
            z = (1-z)/2*Lc;
            Ld = (int)((Lz-1)/kPI*acos(1-2*d/Lc));
            //d = (1-cos(kPI*Ld/(Lz-1)))/2.0*Lc;
            for(int i=0; i<Ld; ++i) 
                array(i) = (1.0+cos(kPI*z(i)/d))/2.0;
            for(int i=0; i<Lx; ++i)
                for(int j=0; j<Ly; ++j)
                    for(int k=0; k<Lz; ++k) {        
                        phiw(i, j, k) = array(k);
    ...
    CMatFile mat;
    mat.matInit("mask.mat","w");
    mwSize dim_array[3]={Lx, Ly, Lz};
    blitz::Array<double, 3> data(Lx, Ly, Lz, fortranArray);
    data = phiw;
    mat.matPut("mask",data.data(),data.size()*sizeof(double),3,dim_array,mxDOUBLE_CLASS,mxREAL);
    mat.matRelease(); 
}
