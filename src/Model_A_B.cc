...
void Model_A_B::init(){
    ...
    sA = vMs(0);
    sC = vMs(1);
    dsA = _cfg.length_ratio()/(sA-1); 
    dsC = 1.0/(sC-1); //length_ratio is length ration of grafted homopymer C over confined homopolymer A
...
/***************************calculate specific segment**********************************/
    ...
    phia_end(all, all, all) = fA/QA*qsA(0, all, all, all)*qsAc(sA-1, all, all, all)/_cfg.length_ratio();
    phia_middle(all, all, all) = fA/QA*qsA((sA-1)/2, all, all, all)*qsAc((sA-1)/2, all, all, all)/_cfg.length_ratio(); 
    ...
    Grid g;
    g = chiN * (*phiA) * (*phiC);
    E = g.quadrature();              //interaction between homopolymer brush and homopolymer matrix

    g = (*wC) * (*phiC);
    S_C = fC*(log(fC/QC)-1) - g.quadrature();      //entropy of brush C

    g = (*wA) * (*phiA);
    S_A = (1-fC)*(log((1-fC)/QA)) - g.quadrature();   //entropy of homopolymer matrix A

    g = (*phiA_middle) * log((*phiA_middle)/_cfg.length_ratio());
    S_Atrans = g.quadrature();        //translational of homopolymer matrix A

    S_Aconf = S_A - S_Atrans;         //configuratial of homopolymer matrix A
}
...
double Model_A_B::Hs() const{
    //double Hs1 = -fA/_cfg.length_ratio()*log(QA)-fC*log(QC);
    //double Hs2 = fA/_cfg.length_ratio()*(log(fA/_cfg.length_ratio())-1)+fC*(log(fC)-1);
    //double mu = _cfg.length_ratio()*log(fC/QC) + log(fA/_cfg.length_ratio()/QA);
    //double Hs3 = -mu * fA/_cfg.length_ratio();
    //return Hs1+Hs2+Hs3;
    //return log(fC/QC)-fC-fA/_cfg.length_ratio();
    double Hs = fA/_cfg.length_ratio()*log(fA/QA)+fC*(log(fC/QC)-1);
    return Hs;
}
...
void Model_A_B::save_density(const string file){
    ...
    if(!mat.queryStatus()){
        ...
        mat.matPutScalar("E", E);
        mat.matPutScalar("S_C", S_C);
        mat.matPutScalar("S_A", S_A);
        mat.matPutScalar("S_Aconf", S_Aconf);
        mat.matPutScalar("S_Atrans", S_Atrans);
        mat.matRelease();
    }
}
...
/*****************calculate delta function**********************/
    ...
    switch(_cfg.dim()) {
    	...
    	{                              // brackets is necessary for initialization in switch case
    		...
    		arr = 2.0/sqrt(2.0*kPI*alpha) * exp(-pow2(x)/2/alpha) + x*x*exp(-_cfg.a()*_cfg.a()/2/alpha)/(2*alpha*sqrt(2.0*kPI*alpha)); 
    	...
    	{                              
    		...
    		y = 0.5 * (1-y) * (_cfg.b());
    		delta.resize(Lx, Ly, Lz);
            arr = 2.0/sqrt(2.0*kPI*alpha) * exp(-pow2(y)/2/alpha) + y*y*exp(-_cfg.b()*_cfg.b()/2/alpha)/(2*alpha*sqrt(2.0*kPI*alpha)); 
    	...
    	{                              
    		...
    		z = 0.5 * (1-z) * (_cfg.c());
    		delta.resize(Lx, Ly, Lz);
            arr = 2.0/sqrt(2.0*kPI*alpha) * exp(-pow2(z)/2/alpha) + z*z*exp(-_cfg.c()*_cfg.c()/2/alpha)/(2*alpha*sqrt(2.0*kPI*alpha));  
