 ...
 * =====================================================================================
 */
//Notice: For "run_scftInString_changingSize(st)" data.mat = defect, ini file, defect.mat&vLam.mat OR stringData.mat
//        is necessary, and gridInitType = File
//        For "run_scftInString(st)" gridInitType = DATA
...
void run_string(string config_file) {
    ...
    } //do not construct ETDRK4 frequently in string iterations, it is time-consuming.
...
void initialize_string() {
    ...
    //save_data(); //for field initialization in scft of first string 
    CMatFile mat;
    ...
    blitz::Array<double, 4> data(m, Nxx, Nyy, Nzz, fortranArray);
    data = wa;
...
void run_scftInString_changingSize(int st) {
    ...
    /*********************import data of last string***************/
    Array<double, 4> tmpwa(m, Nxx, Nyy, Nzz);
    Array<double, 4> tmpwb(m, Nxx, Nyy, Nzz);
    Array<double, 4> tmpphia(m, Nxx, Nyy, Nzz);
    Array<double, 4> tmpphib(m, Nxx, Nyy, Nzz);
    CMatFile mat;
    mat.matInit("./result.mat", "r");
    mat.matGetArray("wa", tmpwa.data(), tmpwa.size()*sizeof(double));
    mat.matGetArray("wb", tmpwb.data(), tmpwb.size()*sizeof(double));
    mat.matGetArray("phia", tmpphia.data(), tmpphia.size()*sizeof(double));
    mat.matGetArray("phib", tmpphib.data(), tmpphib.size()*sizeof(double));
    mat.matRelease();
    /**************************************************************/
    for (int s=0; s<m; s++) {
        cout << endl;
        ...
        cout << "**************************************************" << endl;
        cout << endl;
        /*******************write out data.mat for field initialization**************************/
        cout << "Prepare the file for field initialization" << endl;

        /*CMatFile mat;
        mat.matInit("data.mat","w");
        mwSize Nx = (mwSize)Nxx;
        mwSize Ny = (mwSize)Nyy;
        mwSize Nz = (mwSize)Nzz;
        mwSize dim_array[3]={Nx, Ny, Nz};
        blitz::Array<double, 3> data(Nxx, Nyy, Nzz, fortranArray);
        cout << "polystring_AB 316" << endl;
        data = tmpwa(s+1, all, all, all);
        mat.matPut("wA",data.data(),data.size()*sizeof(double),3,dim_array,mxDOUBLE_CLASS,mxREAL);
        cout << "polystring_AB 319" << endl;
        data = tmpwb(s+1, all, all, all);
        mat.matPut("wB",data.data(),data.size()*sizeof(double),3,dim_array,mxDOUBLE_CLASS,mxREAL);
        data = tmpphia(s+1, all, all, all);
        mat.matPut("phiA",data.data(),data.size()*sizeof(double),3,dim_array,mxDOUBLE_CLASS,mxREAL);
        data = tmpphib(s+1, all, all, all);
        mat.matPut("phiB",data.data(),data.size()*sizeof(double),3,dim_array,mxDOUBLE_CLASS,mxREAL);
        mat.matRelease();
        cout << "polystring_AB 327" << endl;*/
        /*****************************************************************************************/
        //pmodel->input_AField(wa(s,all,all,all));  //field initialization of single scft calculation
        //pmodel->input_BField(wb(s,all,all,all));
        //pmodel->init_data_field();
        cfg.a(stringSize(s));
        cout << "polystring_AB 333" << endl;
        cfg.save(config_file); //this is necessary for write data from memory to disk
        cout << "polystring_AB 335" << endl;
        scft sim(config_file, pmodel);
        cout << "polystring_AB 337" << endl;
        sim.solve_scft_string(cfg.a(), cfg.b(), cfg.c());  //reset pmodel inside
        cout << "polystring_AB 339" << endl;
        H(st,s) = pmodel->H();
        cout << "polystring_AB 341" << endl;
